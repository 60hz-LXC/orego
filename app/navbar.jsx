...
export default function Navbar() {
  ...
  return (
    <div className="flex flex-row-reverse fixed h-16 bg-black bg-opacity-90 w-full border-0 items-center justify-between pl-9 shadow-2xl z-50 font-Comfortaa">
      ...
      <div className={`fixed top-0 mt-16 right-0 h-screen w-screen bg-black bg-opacity-90 z-40 transform ${isOpen ? 'translate-x-0' : 'translate-x-full'} transition-transform duration-300 ease-in-out`}>
        <nav className="text-white text-2xl space-y-6 p-6 lg:p-20">
          {menuItems.map((item, index) => (
            <div
              key={item.name}
              className={`flex flex-row group transition-all duration-300 ease-in-out transform ${menuItemsVisible ? 'translate-x-0 opacity-100' : '-translate-x-10 opacity-0'}`}
